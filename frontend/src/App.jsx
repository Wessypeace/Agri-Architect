...
function App() {
  ...
  const handleSharePlan = async () => {
    ...
    setIsSaving(true);
    try {
      const response = await axios.post('https://agri-architect-backend.onrender.com/api/save-plan', { formData, results });
  ...
  return (
    <>
      <Navbar resultsVisible={!!results} t={t} />
      <main>
        <header id="header">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
              <select id="language" name="language" value={lang} onChange={(e) => { setLang(e.target.value); localStorage.setItem('lang', e.target.value); }}>
                <option value="en">English</option>
                <option value="fr">Français</option>
              </select>
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                <label htmlFor="unit" style={{ fontSize: '0.9rem' }}>{t('settings.units')}</label>
                <select id="unit" value={unit} onChange={(e) => { setUnit(e.target.value); localStorage.setItem('unit', e.target.value); }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <select id="currency" value={currency} onChange={(e) => { setCurrency(e.target.value); localStorage.setItem('currency', e.target.value); }}>
